...
with st.sidebar.form("food_form", clear_on_submit=True):
    ...
    submitted = st.form_submit_button("List Food Item")
...
with tab3:
    ...
    st.dataframe(pd.read_sql_query("SELECT Location as City, COUNT(Food_ID) as Total_Listings FROM food_listings GROUP BY Location ORDER BY Total_Listings DESC;", conn))
    
    st.write("**Total Available Food Quantity (Sum):**")
    st.dataframe(pd.read_sql_query("SELECT SUM(Quantity) as Total_Available_Food FROM food_listings;", conn))
